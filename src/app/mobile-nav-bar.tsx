...
function HamburgerIcon({
  ...
  const getColor = () => (isOpen ? "bg-slate-700" : "bg-white");
  const ifOpen = (className: string) => (isOpen ? className : "");
      ...
      <button className="group relative" onClick={handleClick}>
        <div className="relative flex h-[50px] w-[50px] transform items-center justify-center overflow-hidden rounded-full transition-all duration-200">
          <div className="flex h-[20px] w-[20px] origin-center transform flex-col justify-between overflow-hidden transition-all duration-300">
            <div
              className={`h-[2px] w-7 origin-left transform ${getColor()} transition-all duration-300 ${ifOpen("translate-x-10")}`}
            ></div>
            <div
              className={`h-[2px] w-7 transform rounded ${getColor()} transition-all delay-75 duration-300 ${ifOpen("translate-x-10")}`}
            ></div>
            <div
              className={`h-[2px] w-7 origin-left transform ${getColor()} transition-all delay-150 duration-300 ${ifOpen("translate-x-10")}`}
            ></div>
            ...
            >
              <div
                className={`absolute h-[2px] w-5 rotate-0 transform ${getColor()} transition-all delay-300 duration-500 ${ifOpen("rotate-45")}`}
              ></div>
              <div
                className={`absolute h-[2px] w-5 -rotate-0 transform ${getColor()} transition-all delay-300 duration-500 ${ifOpen("-rotate-45")}`}
              ></div>
